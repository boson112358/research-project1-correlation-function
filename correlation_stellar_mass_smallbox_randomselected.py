...
for i in range(0, len(data.ids.id)):
	if 1e+10 < stellar_masses.to_value(unyt.msun)[i] < 3.16e+10:
		id1.append(i)
print(len(id1))
slice1 =  random.sample(id1,a) 

for i in range(0,a):
	l = slice1[i]
	position1.append([halo_xpositions.to_value(unyt.Mpc)[l], halo_ypositions.to_value(unyt.Mpc)[l], halo_zpositions.to_value(unyt.Mpc)[l]])

for i in range(0, len(data.ids.id)):
	if 3.16e+10 < stellar_masses.to_value(unyt.msun)[i] < 1e+11:
		id2.append(i)
print(len(id2))
slice2 = random.sample(id2,b)

for i in range(0,b):
	l = slice2[i]
	position2.append([halo_xpositions.to_value(unyt.Mpc)[l], halo_ypositions.to_value(unyt.Mpc)[l], halo_zpositions.to_value(unyt.Mpc)[l]])

for i in range(0, len(data.ids.id)):
	if 1e+11 < stellar_masses.to_value(unyt.msun)[i] < 3.16e+11:
		id3.append(i)
print(len(id3))
slice3 = random.sample(id3,c)

for i in range(0,c):
	l = slice3[i]
	position3.append([halo_xpositions.to_value(unyt.Mpc)[l], halo_ypositions.to_value(unyt.Mpc)[l], halo_zpositions.to_value(unyt.Mpc)[l]])
...
plt.legend()
plt.savefig('correlation_stellar_Mass3_N_2000_1000_periodic_smallbox_randomsample.png')
